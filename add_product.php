...
<?php
 if(isset($_POST['add_product'])){
   ...
   if(empty($errors)){
     ...
     // If no media_id selected, set to '0'
     ...
     */
     ...
     // 2) PostgreSQL "Upsert" if 'name' is unique:
     $query  = "INSERT INTO products (name, quantity, buy_price, sale_price, categorie_id, media_id, date)";
     $query .= " VALUES ('{$p_name}', '{$p_qty}', '{$p_buy}', '{$p_sale}', '{$p_cat}', '{$media_id}', '{$date}')";
     $query .= " ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name";
     // Or update more columns if needed:
     ...
     if($db->query($query)){
       ...
       $session->msg('d',' Sorry, product insert failed!');
       redirect('add_product.php', false);
     }
  ...
  <div class="col-md-8">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong><span class="glyphicon glyphicon-th"></span><span>Add New Product</span></strong>
      </div>
